...
`taskA/demos/demo_NN.npz` 입니다. 한 파일에 `meta`(장면 설명), `joint_pos`(관절 19),
`actions`(관절 명령), `base_joints`(바퀴 6), `base`(베이스 x·y·yaw), `obj/crate`(바구니
자세), `segment`(프레임마다 어느 토막인지)가 들어 있습니다. `meta.recorded` 에 그 판이
실제로 어느 장면에서 찍혔는지가 적혀 있습니다.
