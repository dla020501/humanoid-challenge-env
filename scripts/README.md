...
* 서 있는가는 뒷줄 같은 상품의 z 축과 45° 안, 방향은 뒷줄 같은 상품 기준 ±90° 안(서 있을 때만), 상자 제자리는
  파란 상자가 처음 자리에서 2 cm 넘게 안 밀렸을 때, 다른 상품 그대로는 원래 진열된 상품이 넘어지거나 15° 넘게
  기울거나 5 mm 넘게 밀리지 않았을 때입니다 (2026-09-07). 값은 `taskB/taskb_score.py` 의 `THRESHOLD` 에 있습니다.
* 파란 상자가 탁자에서 떨어지거나 탁자가 10 cm 넘게 움직이면 **그 조합의 채점이 그 순간 중단**되고
  그때까지의 점수가 최종입니다. 재생에서는 상자 낙하만 봅니다 -- 기록에 탁자 위치가 없어 탁자 이동은
  심사 서버가 `update(table_pos=...)` 로 줄 때만 잽니다.
