...
| 데이터 묶음 | 내용 | 편수 | 파일 위치 |
| --- | --- | --- | --- |
| **`--set gt`** | 실제 평가 기준과 동일 형식 — **잡기 → QR 인식 → 내려놓기**를 계산대 위 **상품 3개에 대해 연속 3번** 수행하는 정답 궤적 | **이번 배포에는 없습니다** (다시 수집 중) | — |
...
기준이어야 합니다. 동봉된 7편은 LeRobot 형식이 아니라 폴더 형식이므로 `--set` 으로 틉니다.
...
정답 궤적(`--set gt`)은 새 구성으로 다시 수집하는 중이라 이번 배포에는 빠져 있습니다. 준비되는 대로 올리겠습니다.
...
| 옵션 | 설명 |
| --- | --- |
| `--set gt` 또는 `--set single` | 재생할 데이터 세트 선택 (기본값: gt). 정답 궤적은 지금 비어 있어 `--set single` 로 트십시오 |
| `--seed N` | 해당 세트 내 에피소드 번호 지정 (single: 0~3 / 기본값: 0) |
