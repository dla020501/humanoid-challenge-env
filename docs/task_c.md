...
* **빨간 띠 (계산대 상판):** 상판(높이 0.9035 m) 위 로봇 기준 전방 11~50 cm · 좌측 -1~57 cm 를 폭 20 mm 테이프로 두른 자리입니다. **상품은 이 안에서만 스폰됩니다.**
* **상품 3개:** 8종 중 시드가 고른 3개가 띠 안에 놓입니다. **슬롯 0 이 집어야 할 목표 상품**이고 나머지 둘은 배경입니다. 셋 다 **QR 면이 로봇 기준 우측(월드 -Y)을 향하고**, 원통형은 세워서·상자형은 눕혀서 놓이며 간격은 최소 10 cm 입니다.
...
* *참고:* 스폰 뒤 3초간 물리로 정착시키고, 띠 밖으로 밀리거나 넘어지거나 QR 방위가 3° 넘게 틀어지거나 간격이 10 cm 미만이면 같은 시드 안에서 최대 50회 재배치합니다. 출력 좌표는 **정착 후 실측값**입니다.
...
| `--set gt --seed` | 처리 순서 (슬롯 0 → 1 → 2) | 프레임 / 시간 | 슬롯별 들림 | QR |
| --- | --- | --- | --- | --- |
| **0** | 오뚜기 → 예감 → 삼양 | 9,242 / 308초 | 265.8 · 262.2 · **68.4 (실패)** mm | 2회 |
| **1** | 삼양 → 오뚜기 → 예감 | 7,567 / 252초 | 208.2 · 280.9 · 334.0 mm | 3회 |
| **2** | 삼양 → 예감 → 오뚜기 | 8,236 / 275초 | 210.1 · 262.2 · 360.1 mm | 5회 |

동봉된 `gt` 3편은 만점짜리가 아닙니다. 시드 0 은 마지막 슬롯(삼양 불닭 컵)을 들지 못해 68.4 mm
밖에 뜨지 않고 띠 밖에 놓입니다(파지가 어긋나는 지점은 250초 이후). 시드 2 는 상품 셋을 모두
집어 스캐너에 댔지만 오뚜기 컵누들 불닭을 157 mm·173 mm 자리에서 읽혀 15 cm 규정 밖입니다(§6).
...
| `--set single --seed` | 상품 | 프레임 / 시간 | 들림 | QR |
| --- | --- | --- | --- | --- |
| **0** | 코카콜라 제로 캔 (`cocacola_zero`) | 2,097 / 70초 | 257.3 mm | 2회 |
| **1** | 오뚜기 컵누들 불닭 (`ottogi_cupnoodle_buldak`) | 1,928 / 64초 | 212.7 mm | 2회 |
| **2** | 프링글스 오리지널 소 (`pringles_original_small`) | 3,099 / 103초 | 228.9 mm | 2회 |
| **3** | 예감 오리지널 (`yegam_original`) | 1,983 / 66초 | 249.3 mm | 1회 |
...
이 값들은 수집 당시 실측입니다. 동봉된 학습 데이터의 판독이 횡이탈 35~38 mm 에서 일어났으므로 이보다 좁은 창을 쓰면 정상 시연도 기각됩니다. 축거리 상한 150 mm 는 평가 기준의 「빔 출발선에서 15 cm」와 같습니다. 빔 자국·LED·띠 점등을 켜는 시각 판정은 별개 조건(횡이탈 6 mm)이라 겸하지 않습니다.
...
| 판 | 점수 | 비고 |
|---|---|---|
| `gt` 시드 0 | 38.0 / 51 | 마지막 슬롯이 조준·판독·배치에서 떨어집니다 (§5) |
| `gt` 시드 1 | 51.0 / 51 | |
| `gt` 시드 2 | 41.0 / 51 | 오뚜기 컵누들 불닭을 15 cm 밖에서만 읽혀 인정되지 않습니다 (§5) |
