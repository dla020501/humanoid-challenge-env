...
scripts/      주최측 제공 스크립트 — 씬 생성기(task 별)와 데모 평가 서버.
  ...
  taskA/          과제 A 에 필요한 파일. 시연 기록 3판, 채점기
...
과제 A의 평가는 **6개 항목, 총 21점 만점**으로 구성됩니다. 시도 3회를 합산한 63점이 최종 만점입니다. 시연 재생 스크립트를 실행하면 내부적으로 `scripts/taskA/scorer/taska_score.py`가 연동되어, 각 구간이 끝나는 시점에 해당 구간의 채점 결과를 터미널에 출력합니다.
...
| 손을 뗀 뒤 6초 동안 잘 놓여 있었는가 | 손 뗀 뒤 6초 | 4 |
...
* **항목:** 바구니 들어 올림, 목적지 도착 및 정지, 책상 상판에 얹음
* **판정:** 에피소드가 진행되는 동안 해당 조건을 최초로 만족하는 순간 점수가 부여되며, 이후 상태가 변하더라도 부여된 점수는 회수되지 않습니다.

2. **특정 시점의 상태 (State-check)**
* **항목:** 도착 시점의 파지 여부, 그리퍼를 연 뒤 6초 동안의 안착 상태
* **판정:** 해당 시점의 상태만을 단 한 번 평가합니다. 6초 항목은 그 구간에서 **가장 나쁜 순간**을 기준으로 판정합니다.

* **충돌 항목은 판 전체를 봅니다.** 로봇 또는 로봇이 든 바구니의 발자국(위에서 내려다본 바닥 면적)이 매장 집기의 영역과 겹치면 실패입니다. **허용 오차가 없습니다** — 1 mm만 겹쳐도 실패이고, 겹치지 않으면 통과입니다.

> **떨림은 채점 항목이 아닙니다.** 로봇이 주행 중 제자리에 멈춰 서 있어도, 목적지에 도착하기만 하면 점수는 동일합니다.

#### 「쥐고 있는가」의 판정 기준

로봇이 바구니를 쥐고 있는지를 **접촉력이 아닌 위치로** 판정합니다. 손가락 끝이 바구니 표면에서 40 mm 이내에 있고, 두 손가락 사이가 60 mm 이내로 좁혀져 있으면 쥔 것으로 간주합니다.

접촉력으로 측정하려면 물리 연산을 다시 수행해야 하는데, 장시간 주행에서는 그 재계산이 원본 기록과 다르게 흘러가 바구니를 놓치는 경우가 발생합니다. 위치로 측정하면 기록만으로 판정이 가능합니다. **신뢰도는 검증되었습니다** — 7,041 프레임에서 접촉력 기반 판정과 **99.9%** 일치했습니다(불일치 9프레임).

#### 동봉된 데모 데이터 3종 채점 결과

(기본 내장된 채점기로 측정한 기준입니다)

| `--seed` | 좌석 | 소요 시간 | 최종 점수 | 못 받은 항목 |
| --- | --- | --- | --- | --- |
| **0** | 0 | 147 초 | 21 / 21 | - |
| **2** | 10 | 135 초 | 21 / 21 | - |
| **6** | 6 | 189 초 | 21 / 21 | - |
...
채점 스크립트(`taska_score.py`)는 Isaac Sim 시뮬레이터 구동 없이, Numpy 환경만 구성되어 있다면 기록된 `.npz` 파일을 통해 독립적으로 실행해 볼 수 있습니다.

```bash
python3 scripts/taskA/scorer/taska_score.py scripts/taskA/demos/demo_00.npz
```

여러 판을 한 번에 넣으면 합계까지 출력합니다.

```bash
python3 scripts/taskA/scorer/taska_score.py scripts/taskA/demos/demo_*.npz
```

*각 평가 항목별 구체적인 측정 방식과 임계값(Threshold) 수치는 해당 파이썬 파일 내부의 `POINTS` 및 `thresholds()` 변수, 그리고 `scripts/README.md` 문서에서 상세히 확인할 수 있습니다.*
