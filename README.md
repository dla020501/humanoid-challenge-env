...
| | 무엇이 달라지나 | 규칙 |
|---|---|---|
| **좌석** | 로봇과 바구니가 열두 곳 중 어디서 출발하는가 | `좌석 = (seed × 7919) % 12` |
| **목표 진열대** | 어떤 상품 21 개가 서 있고 앞줄 어느 세 칸이 비는가 | seed 에서 뽑은 진열대씨앗 |
| **기타 진열대** | 곤돌라 12 개에 무엇이 서 있는가 | seed 에서 뽑은 매장씨앗 (열두 벌 중 하나) |
| **스툴** | 로봇이 쓰지 않는 두 탁상의 스툴 여덟 개가 어떻게 흩어지는가 | seed |

**진열대의 자리는 달라지지 않습니다.** 곤돌라·냉장고·계산대는 늘 같은 곳에 있고, 바뀌는
것은 선반에 선 물건뿐입니다. 그래서 장애물의 모양은 seed 를 타지 않고, 정책이 보는 그림만
매번 달라집니다.
...
--check            # 좌석 기하와 진열과 매장 USD 만 검사하고 끝냅니다. Isaac Sim 을 안 띄워 1 초
```
...
기타 진열대는 **미리 구워 둔 열두 벌**(`scripts/taskA/stores/`) 중 하나를 씁니다. 매번 새로
채우지 않고 구워 둔 것을 쓰는 이유가 있습니다 — 진열기가 부동소수점 가중치로 뽑는 탓에
**같은 씨앗이라도 CPU 가 다르면 다른 매장이 나옵니다**(실측: 한 기계에서 22 종 103 개,
다른 기계에서 17 종 105 개). 파일을 읽는 데는 기계가 끼어들 자리가 없으므로, 위의 약속
"같은 seed 면 어디서 돌려도 같은 장면" 이 곤돌라에서도 지켜집니다.
...
### 정답 주행 세 판 틀어 보기 (Replay)

`task_a_demo.py` 가 정지된 시작 화면만 보여 준다면, `task_a_replay.py` 는 로봇이 실제로
바구니를 집어 매장을 가로질러 책상에 내려놓는 **전 과정**을 보여 줍니다. 저장소에 주최
측이 미리 수집한 정답 주행 세 판이 들어 있고, **셋 다 평가표 만점(21 / 21)** 입니다.
...
무엇이 들어 있는지만 보려면 (Isaac Sim 을 안 띄웁니다 — 1 초):
...
`--from_seconds 150` 으로 놓기만, `--substeps 8` 로 더 부드럽게, `--hz 0` 으로 최대 속도로
볼 수 있습니다.

#### 재생 방식 — 물리로 굴리지 않고 자세를 써 넣습니다

기록해 둔 궤적을 말 그대로 재생합니다. 관절 각도와 베이스 자세, 바구니 자세를 프레임마다
기록에서 그대로 읽어 써 넣습니다. 과제 B 의 `task_b_replay.py` 와 같은 방식입니다.

같은 방식인 것이 중요합니다. 이 기록은 **GPU 물리와 컴플라이언트 바닥** 위에서 초당 10
프레임으로 찍혔고, 이 저장소의 데모는 CPU 물리에 기본 바닥입니다. 그 차이는 물리가 결과를
만들 때만 문제가 됩니다 — 실측으로, 같은 계획을 GPU 물리로 돌리면 바구니를 끝까지 들고
갔고 CPU 물리에서는 목적지 2.5 m 앞에서 떨어뜨렸습니다. 자세를 써 넣는 재생에는 그
갈림길이 없습니다.

> 재생되는 매장의 진열은 `--seed` 가 정하는 것이라, 그 판을 **찍을 때의 진열과 다를 수
> 있습니다**. 궤적에는 영향이 없습니다 — 바구니가 목표 진열대에서 최소 0.45 m 떨어져
> 지나가고(세 판 모두 실측), 곤돌라 상품에는 콜라이더가 없습니다. 찍힌 그대로 보시려면
> `--as_recorded` 를 주십시오.
