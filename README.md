...
* *참고 (기타 진열대의 재현성):* 곤돌라 진열은 **사전에 생성해 둔 12벌**(`scripts/taskA/stores/`) 중 하나를 사용합니다. 실행 시점에 매번 새로 배치하지 않는 이유는, 배치 알고리즘이 부동소수점 가중치를 사용하는 탓에 **동일한 시드라도 CPU가 다르면 다른 결과가 나오기 때문입니다** (실측: A 장비 22종 103개 / B 장비 17종 105개). 파일을 읽어 오는 방식에는 장비 차이가 개입할 여지가 없으므로, 위의 "동일 시드 = 동일 환경" 이 곤돌라에서도 보장됩니다.
...
기록해 둔 궤적을 말 그대로 재생합니다. 로봇의 관절 각도와 베이스 위치, 바구니의 자세를 프레임마다 기록에서 그대로 읽어 씁니다.
...
### 6. 재생 방식 — 물리 연산이 아닌 자세 기록 방식

Task A의 재생은 **기구학(Kinematic) 재생**입니다. 물리 엔진이 결과를 만들어내는 것이 아니라, 매 프레임의 관절 각도와 자세를 기록에서 그대로 써 넣습니다. Task B의 `task_b_replay.py`와 동일한 방식이며, Task A에서는 이 점이 특히 중요합니다.

동봉된 기록은 **GPU 물리 연산과 컴플라이언트(무른) 접촉 바닥** 환경에서 초당 10프레임으로 수집되었으나, 본 저장소의 데모 환경은 CPU 물리 연산에 기본 바닥입니다. 이 차이는 **물리 엔진이 결과를 결정할 때만** 문제가 됩니다. 실측 결과 동일한 계획을 GPU 물리에서 실행하면 바구니를 목적지까지 운반했으나, CPU 물리에서는 목적지 2.5 m 앞에서 낙하시켰습니다. 자세를 직접 써 넣는 재생 방식에는 이러한 분기가 발생하지 않습니다.

다만 기구학 재생이 반드시 충족해야 하는 조건이 3가지 있으며, 스크립트가 이를 자체 검사합니다.

1. **관절은 이름으로 매칭합니다.** 기록에는 관절 19개와 바퀴 6개가 있고 로봇은 총 31개입니다. 순서로 매칭하면 잘못 대응되어도 그럴듯한 동작이 나오므로, 가장 발견하기 어려운 오류가 됩니다. 채우지 못하는 관절이 하나라도 있으면 스크립트가 중단됩니다.
2. **그리퍼의 2~4번 마디를 매 프레임 1번 마디 값으로 채웁니다.** 기록에는 1번 마디만 존재합니다. 이를 생략하면 그리퍼가 벌어진 상태로 재생되며(실측 68.9~80.6 mm 대 정상 10.0 mm), 화면상 아무것도 쥐지 않은 채 바구니가 따라다니는 형태가 됩니다.
3. **루트 쿼터니언을 새로 생성하지 않습니다.** FFW-SG2의 루트 링크는 항등 자세에서 직립 상태가 아니므로, yaw 쿼터니언을 생성해 덮어쓰면 로봇이 정확히 90도 옆으로 눕습니다. 로봇이 실제로 스폰되어 직립한 상태의 쿼터니언을 읽어 두고, 그것을 월드 수직축 기준으로 회전시킵니다.

**재생으로 확인할 수 없는 것 — 접촉입니다.** 손가락이 바구니를 눌러 딸려 오는 것이 아니라 바구니 또한 기록된 위치에 놓입니다. "이 파지가 미끄러지지 않고 버티는가"는 이 화면이 답할 수 있는 질문이 아닙니다. 그 질문은 해당 판을 실제로 실행하여 이미 답한 것이며, 3판 모두 만점으로 종료된 기록입니다.

> ⚠️ **재생되는 매장의 진열은 수집 당시와 다를 수 있습니다.**
> 진열은 `--seed`가 결정하므로 `task_a_demo.py`와 동일한 매장이 스폰되지만, 이 3판을 수집할 당시의 매장은 그와 달랐습니다(seed 0·6은 재진열 없이 목표 진열대가 빈 상태, seed 2는 다른 시드). **궤적에는 영향이 없습니다** — 자세를 직접 써 넣는 재생 방식이고, 곤돌라 상품에는 콜라이더가 없으며, 바구니 중심이 목표 진열대로부터 최소 0.45 m(로봇 발자국 기준 0.52 m) 떨어져 통과합니다(3판 모두 실측). 수집 당시 그대로 보려면 `--as_recorded` 옵션을 사용하십시오.
